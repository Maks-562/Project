...
class Tree:
    ...
    def update_weights(self, rootnode, learning_rate, nabla_C, L, lam):
        ...
        if rootnode.left == None and rootnode.right == None:
            rootnode.activation += (
                -learning_rate * nabla_C[rootnode.index - 1]
            ...
            while queue:
                ...
                if node.layer < self.layers:
                    ...
                    for child, mode in zip(children, modes):
                        ...
                        alpha_da_df = np.dot(
                            node.error, differentiate_parent(node, mode)
                        )
                        df_dw = (1 / node.weight) * (
...
def homogenise(phase1, phase2):
    ...
    D_r[1] = (
        phase1.f * phase1.compliance[1] * phase2.compliance[0]
        + phase2.f * phase2.compliance[1] * phase1.compliance[0]
    ) / gamma
    D_r[2] = (
        phase1.f * phase1.compliance[2] * phase2.compliance[0]
        + phase2.f * phase2.compliance[2] * phase1.compliance[0]
    ) / gamma
    D_r[3] = (
        phase1.f * phase1.compliance[3]
        + phase2.f * phase2.compliance[3]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.compliance[1] - phase2.compliance[1]) ** 2
    )
    D_r[4] = (
        phase1.f * phase1.compliance[4]
        + phase2.f * phase2.compliance[4]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.compliance[2] - phase2.compliance[2])
        * (phase1.compliance[1] - phase2.compliance[1])
    )
    D_r[5] = (
        phase1.f * phase1.compliance[5]
        + phase2.f * phase2.compliance[5]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.compliance[2] - phase2.compliance[2]) ** 2
    )
...
def error_relative(D_dns, D_h):
    ...
    if np.shape(D_dns) == (3, 3):
        ...
        return np.linalg.norm(convert_vectorised(D_dns) - convert_vectorised(D_h)) / (
...
def calc_error(parent):
    ...
    for child_index, child in enumerate(children, start=1):
        for i in range(6):
            diff = differentiate_parent(
                parent, f"D_{child_index}_{element_map[str(i)]}"
            )
