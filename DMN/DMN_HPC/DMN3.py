...
class Tree:
        ...
        while queue:
            ...
            if node.layer == self.layers:
                ...
                if node.index % 2 == 1:
                    ...
                    node.rotated_compliance = convert_matrix(
                        R(-node.theta) @ convert_vectorised(phase1) @ R(node.theta)
                    )
                    ...
                    node.rotated_compliance = convert_matrix(
                        R(-node.theta) @ convert_vectorised(phase2) @ R(node.theta)
                    )
                    # node.compliance =np.zeros(6)
    ...
    def homogenise_system(self, rootnode, phase1, phase2):
        ...
        if rootnode.left is not None and rootnode.right is not None:
            ...
            rootnode.rotated_compliance = convert_matrix(
                R(-rootnode.theta)
                @ convert_vectorised(rootnode.compliance)
                @ R(rootnode.theta)
            )
    ...
    def SGD(
        ...
        for i in range(0, epochs):
            ...
            for j in range(0, len(training_data), mini_batch_size):
                ...
                self.update_weights(
                    D_r, learning_rate, nabla_C_z, nabla_C_theta, L, lam=lam
                )
            ...
            cost_array.append(cost_funct / len(training_data))
            ...
            if i % 15 == 0 and i > 0:
                ...
                if std_error < 10**-4:
                    print(
                        f" The training process has terminated as the deviation in the last 15 error values is: {std_error:.3f}"
                    )
                    return error_array
            ...
            if error_array[i] < min_validation_error:
                ...
                self.save_model(
                    D_r, metadata, error_train, error_array, filename=filename
                )
    ...
    def update_weights(self, rootnode, learning_rate, nabla_C_z, nabla_C_theta, L, lam):
        if rootnode is None:
            return
        self.update_weights(
            rootnode.left, learning_rate, nabla_C_z, nabla_C_theta, L, lam
        )
        self.update_weights(
            rootnode.right, learning_rate, nabla_C_z, nabla_C_theta, L, lam
        )

        rootnode.theta += (
            -learning_rate * nabla_C_theta[rootnode.layer, rootnode.index - 1]
        )

        if rootnode.left == None and rootnode.right == None:
            rootnode.activation += (
                -learning_rate * nabla_C_z[rootnode.index - 1]
                - learning_rate * lam * L[rootnode.index - 1]
            )
    ...
    def regularisation_term(self, rootnode):
        ...
        bottom_activations_derivatives = relu_prime(
            np.array([node.activation for node in bottom_layer])
        )
    ...
    def propagate_weights(self, rootnode):
        ...
        if rootnode.left is None and rootnode.right is None:
            ...
            if rootnode.weight == 0:
                ...
                print(
                    "CHILDREN:",
                    rootnode.left.activation,
                    rootnode.right.activation,
                    rootnode.left.f,
                    rootnode.right.f,
                )
    ...
    def calc_cost_gradient(self, root):
        ...
        while queue:
            ...
            if node.layer < self.layers:
                ...
                sum1 += (
                    np.dot(node.error_alpha, differentiate_parent(node, "f1")) * df[0]
                )
                sum2 += (
                    np.dot(node.error_alpha, differentiate_parent(node, "f2")) * df[1]
                )

                vect_sum1 += df[0]
                vect_sum2 += df[1]

        bottom_activations = relu_prime(
            np.array([node.activation for node in bottom_layer])
        )
            ...
            while queue:
                ...
                if node.layer < self.layers:
                    ...
                    for child, mode in zip(children, modes):
                        ...
                        alpha_da_df = (
                            node.error_alpha
                            @ self.differentiate_parent_sympy(
                                node, mode, diff_wrt_rot=True
                            )
                        )
                        ...
                        df_dw = (1 / node.weight) * (
                            differentiate_weight(child, node_j)
                            - child.f * differentiate_weight(node, node_j)
                        )
    ...
    def calc_cost_gradient_angles(self, root):
        ...
        while queue:
            ...
            if node.right:
                queue.append(node.right)
            # print(node.layer)
            cost[node.layer, node.index - 1] = (
                node.error_delta
                @ differentiate_D_wrt_theta(node.compliance, node.theta)
            )
    ...
    def compression(self, rootnode):
        nodes = self.get_all_nodes(rootnode)

        # for i in range(self.layers):
        #     for j in range(1,2**i+1):
        #         # print(i,j)
        #         nodes.append(self.fetch_node(rootnode,i,j))
        # # print(nodes)
        # if rootnode is None:
        #     return
        # queue = [rootnode]
        ...
        for node in nodes:
            if node.left is not None:
                ...
                if node.right.f == 1.0:
                    node.theta += node.right.theta
                    node.right.left = node
                    node.right.right = node
                    # node.right = None

    def save_model(
        self, rootnode, metadata, training_error, validation_error, filename=None
    ):
        if filename is None:
            filename = f"model_N={self.layers}.pkl"
    ...
    def get_derivatives(self):
        ...
        D_1_11, D_1_12, D_1_13, D_1_22, D_1_23, D_1_33 = sym.symbols(
            "D_1_11 D_1_12 D_1_13 D_1_22 D_1_23 D_1_33"
        )
        D_2_11, D_2_12, D_2_13, D_2_22, D_2_23, D_2_33 = sym.symbols(
        ...
        D_r_22 = (
            f1 * D_1_22 + f2 * D_2_22 - (1 / gamma) * (f1 * f2) * (D_1_12 - D_2_12) ** 2
        )
        D_r_23 = (
            f1 * D_1_23
            + f2 * D_2_23
            - (1 / gamma) * (f1 * f2) * (D_1_13 - D_2_13) * (D_1_12 - D_2_12)
        )
        D_r_33 = (
            f1 * D_1_33 + f2 * D_2_33 - (1 / gamma) * (f1 * f2) * (D_1_13 - D_2_13) ** 2
        )
    ...
    def differentiate_parent_sympy(self, parent, element, diff_wrt_rot=True):
        ...
        derivative_symbolic = self.derivatives_cache[index]
        # print(self.derivatives_cache)
        ...
        subs_symbols = {
            f1: child1.f,
            f2: child2.f,
            D_1_11: child1.rotated_compliance[0],
            D_1_12: child1.rotated_compliance[1],
            D_1_13: child1.rotated_compliance[2],
            D_1_22: child1.rotated_compliance[3],
            D_1_23: child1.rotated_compliance[4],
            D_1_33: child1.rotated_compliance[5],
            D_2_11: child2.rotated_compliance[0],
            D_2_12: child2.rotated_compliance[1],
            D_2_13: child2.rotated_compliance[2],
            D_2_22: child2.rotated_compliance[3],
            D_2_23: child2.rotated_compliance[4],
            D_2_33: child2.rotated_compliance[5],
        }
        ...
        for index, function in enumerate(derivative_symbolic):
            derivative_numeric[index] = function(
                *child1.rotated_compliance,
                *child2.rotated_compliance,
                child1.f,
                child2.f,
            )

        # derivative_numeric[indxs]   = function.subs(subs_symbols)
    ...
    def calc_error_delta(self, parent):
        ...
        for child_index, child in enumerate(children, start=1):
            for i in range(6):
                diff = self.differentiate_parent_sympy(
                    parent, f"D_{child_index}_{element_map[str(i)]}", diff_wrt_rot=True
                )
...
def homogenise(phase1, phase2):
    ...
    gamma = (
        phase1.f * phase2.rotated_compliance[0]
        + phase2.f * phase1.rotated_compliance[0]
    )

    D_r[0] = (phase1.rotated_compliance[0] * phase2.rotated_compliance[0]) / gamma
    D_r[1] = (
        phase1.f * phase1.rotated_compliance[1] * phase2.rotated_compliance[0]
        + phase2.f * phase2.rotated_compliance[1] * phase1.rotated_compliance[0]
    ) / gamma
    D_r[2] = (
        phase1.f * phase1.rotated_compliance[2] * phase2.rotated_compliance[0]
        + phase2.f * phase2.rotated_compliance[2] * phase1.rotated_compliance[0]
    ) / gamma
    D_r[3] = (
        phase1.f * phase1.rotated_compliance[3]
        + phase2.f * phase2.rotated_compliance[3]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.rotated_compliance[1] - phase2.rotated_compliance[1]) ** 2
    )
    D_r[4] = (
        phase1.f * phase1.rotated_compliance[4]
        + phase2.f * phase2.rotated_compliance[4]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.rotated_compliance[2] - phase2.rotated_compliance[2])
        * (phase1.rotated_compliance[1] - phase2.rotated_compliance[1])
    )
    D_r[5] = (
        phase1.f * phase1.rotated_compliance[5]
        + phase2.f * phase2.rotated_compliance[5]
        - (1 / gamma)
        * (phase1.f * phase2.f)
        * (phase1.rotated_compliance[2] - phase2.rotated_compliance[2]) ** 2
    )
...
def cost(D_dns, D_h):
    ...
    return 0.5 * np.linalg.norm(D_dns - D_h) ** 2
...
def differentiate_parent(parent, element, diff_wrt_rot=True):
    if type(parent) != Node:
        raise TypeError("Parent and child must be Node types")
    ...
    if diff_wrt_rot:
        for index, function in enumerate(derivative_symbolic):
            derivative_numeric[index] = function.subs(
                {
                    f1: child1.f,
                    f2: child2.f,
                    D_1_11: child1.rotated_compliance[0],
            ...
            derivative_numeric[index] = function.subs(
                {
                    f1: child1.f,
                    f2: child2.f,
                    D_1_11: child1.compliance[0],
                    D_1_12: child1.compliance[1],
                    D_1_13: child1.compliance[2],
                    D_1_22: child1.compliance[3],
                    D_1_23: child1.compliance[4],
                    D_1_33: child1.compliance[5],
                    D_2_11: child2.compliance[0],
                    D_2_12: child2.compliance[1],
                    D_2_13: child2.compliance[2],
                    D_2_22: child2.compliance[3],
                    D_2_23: child2.compliance[4],
                    D_2_33: child2.compliance[5],
                }
            )
...
def differentiate_fraction(parent, bottom_layer_array):
    ...
    df_dw = np.array(
        [np.zeros(num_bottom_layer_elements), np.zeros(num_bottom_layer_elements)]
    )
    ...
    for index, child in enumerate(children):
        ...
        for node_index, node_j in enumerate(bottom_layer_array):
            ...
            f_grad[node_index] = (1 / parent.weight) * (
                differentiate_weight(child, node_j) - child.f * dw_parent_dw_j
            )
...
def differentiate_D_wrt_theta(D_r, theta):
    if D_r.shape == (6,):
        ...
        D_theta = -R_prime(-theta) @ D_temp @ R(theta) + R(-theta) @ D_temp @ R_prime(
            theta
        )
    else:
        D_theta = -R_prime(-theta) @ D_r @ R(theta) + R(-theta) @ D_r @ R_prime(theta)

    return convert_matrix(D_theta)


# def differentiate_D_wrt_D_r(theta):
...
def differentiate_D_wrt_D_r(node):
    ...
    for i in range(6):
        ...
        for j in range(6):
            ...
            row[j] = (
                R_neg[D_bar_indices[0] - 1, D_r_indices[0] - 1]
                * R_pos[D_bar_indices[1] - 1, D_r_indices[1] - 1]
            )
