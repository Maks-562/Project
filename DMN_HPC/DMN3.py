...
class Tree: 
    ...
    def calc_cost_gradient(self,root):
        ...
        while queue:
            ...
            if node.layer < self.layers:
                ...
                sum1 += np.dot(node.error_alpha,differentiate_parent(node,'f1')) *df[0]
                sum2 += np.dot(node.error_alpha,differentiate_parent(node,'f2')) * df[1]
    ...
    def differentiate_parent_sympy(self,parent, element, diff_wrt_rot=True):
        ...
        subs_symbols = {f1: child1.f, f2:child2.f,
                                                        D_1_11: child1.rotated_compliance[0], D_1_12 :child1.rotated_compliance[1], D_1_13:child1.rotated_compliance[2],
                                                        D_1_22 : child1.rotated_compliance[3], D_1_23 : child1.rotated_compliance[4], D_1_33 : child1.rotated_compliance[5], 
                                                        D_2_11: child2.rotated_compliance[0], D_2_12 :child2.rotated_compliance[1], D_2_13:child2.rotated_compliance[2],
                                                        D_2_22 : child2.rotated_compliance[3], D_2_23 : child2.rotated_compliance[4], D_2_33 : child2.rotated_compliance[5]}
        ...
        for index,function in enumerate(derivative_symbolic):
            derivative_numeric[index] = function(*child1.rotated_compliance,*child2.rotated_compliance, child1.f, child2.f)
...
def homogenise(phase1,phase2):
    ...
    gamma = phase1.f * phase2.rotated_compliance[0] + phase2.f * phase1.rotated_compliance[0]
   
    D_r[0] = (phase1.rotated_compliance[0] * phase2.rotated_compliance[0])/gamma
    D_r[1] = (phase1.f * phase1.rotated_compliance[1] * phase2.rotated_compliance[0] + phase2.f * phase2.rotated_compliance[1] * phase1.rotated_compliance[0])/gamma
    D_r[2] = (phase1.f * phase1.rotated_compliance[2] * phase2.rotated_compliance[0] + phase2.f * phase2.rotated_compliance[2] * phase1.rotated_compliance[0])/gamma
    D_r[3] = phase1.f * phase1.rotated_compliance[3] + phase2.f * phase2.rotated_compliance[3] - (1/gamma) * (phase1.f * phase2.f) * (phase1.rotated_compliance[1] - phase2.rotated_compliance[1])**2
    D_r[4] = phase1.f * phase1.rotated_compliance[4] + phase2.f * phase2.rotated_compliance[4] - (1/gamma) * (phase1.f * phase2.f) * (phase1.rotated_compliance[2]- phase2.rotated_compliance[2]) * (phase1.rotated_compliance[1] - phase2.rotated_compliance[1])
    D_r[5] = phase1.f * phase1.rotated_compliance[5] + phase2.f * phase2.rotated_compliance[5] - (1/gamma) * (phase1.f * phase2.f) * (phase1.rotated_compliance[2]- phase2.rotated_compliance[2])**2
...
def error_relative(D_dns,D_h):
    ...
    if np.shape(D_dns) ==  (3,3):
        ...
        e = np.linalg.norm(convert_vectorised(D_dns)- convert_vectorised(D_h)) / (np.linalg.norm(convert_vectorised(D_dns)))
        # print(D_dns-D_h)
        return e
...
def R_prime(x):
    ...
    r_prime = np.array([[-s, s, root_2*c],
                  [s,-s,-root_2* c],
                  [-root_2*c, root_2*c,-2* s]])
    return r_prime
