...
def fenics_simple_test(E_inclusion, E_bulk, nu_inclusion, nu_bulk, load_type="1"):
    ...
    bc2 = fem.dirichletbc(
        ...
        fem.locate_dofs_topological(
            V, fdim, mesh.locate_entities_boundary(msh, fdim, clamped_boundary2)
        ),
        V,
    )
    ...
    traction_boundary_facets1 = mesh.locate_entities_boundary(
        msh, fdim, traction_boundary1
    )
    traction_boundary_facets2 = mesh.locate_entities_boundary(
        msh, fdim, traction_boundary2
    )

    traction_boundary_facets3 = mesh.locate_entities_boundary(
        msh, fdim, traction_boundary3
    )
    traction_boundary_facets4 = mesh.locate_entities_boundary(
        msh, fdim, traction_boundary4
    )
    ...
    E.x.array[phase1_cells] = np.full_like(
        phase1_cells, E_bulk, dtype=default_scalar_type
    )
    E.x.array[phase2_cells] = np.full_like(
        phase2_cells, E_inclusion, dtype=default_scalar_type
    )

    nu.x.array[phase1_cells] = np.full_like(
        phase1_cells, nu_bulk, dtype=default_scalar_type
    )
    nu.x.array[phase2_cells] = np.full_like(
        phase2_cells, nu_inclusion, dtype=default_scalar_type
    )
    ...
    problem = LinearProblem(
        a, L, bcs=[bc, bc2], petsc_options={"ksp_type": "preonly", "pc_type": "lu"}
    )
...
def calc_eff_elastic_prop(E_bulk, nu_bulk, E_inclusion, nu_inclusion):
    ...
    for i in range(1, 4):
        S, Eps = fenics_simple_test(
            E_inclusion, E_bulk, nu_inclusion, nu_bulk, load_type=str(i)
        )
    ...
    x0 = convert_matrix(
        elasticity_matrix((E_bulk + E_inclusion) / 2, (nu_bulk + nu_inclusion) / 2)
    )
    ...
    D = np.linalg.inv(C)
    D1 = np.linalg.inv(C1)
    D3 = convert_vectorised(
        homogenise(
            convert_matrix(np.linalg.inv(elasticity_matrix(E_inclusion, nu_inclusion))),
            convert_matrix(np.linalg.inv(elasticity_matrix(E_bulk, nu_bulk))),
            0.5,
            0.5,
        )
    )
    ...
    print(
        f"The elastic moduli of the bulk and inclusion are {E_bulk:.2e} Pa and {nu_bulk:.2f} and {E_inclusion:.2e} Pa and {nu_inclusion:.2f} respectively"
    )
